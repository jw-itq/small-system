...
public class CartController {
    ...
    public Result<Object> editCheckAll(@RequestBody Cart cart){
        int result = cartService.editCheckAll(cart.getUserId(),cart.getChecked());

        return new ResultUtil<Object>().setData(result);
    }

    @RequestMapping(value = "/member/cartEdit",method = RequestMethod.POST)
    public Result<Object> cartEdit(@RequestBody Cart cart){
        int result = cartService.cartEdit(cart.getUserId(),cart.getProductId(),cart.getChecked(),cart.getProductNum());
        return new ResultUtil<Object>().setData(result);
    }
    ...
    public Result<List<CartProduct>> getCartList(@RequestBody Cart cart){
        List<CartProduct> result = cartService.getCartList(cart.getUserId());
        return new ResultUtil<List<CartProduct>>().setData(result);
    }
}
