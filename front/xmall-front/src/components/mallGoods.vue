<template>
  <div class="good-item">
    <div style="">
      ...
      <h3 class="sub-title ellipsis" v-html="msg.subTitle">{{msg.subTitle}}</h3>
    ...
    methods: {
      ...
      addCart (id, price, name, img) {
        if (!this.showMoveImg) {     // 动画是否在运动
          if (this.login) { // 登录了 直接存在用户名下，他的意思是存在当前用户中的数据库中进行关联
            addCart({userId: getStore('userId'), productId: id, productNum: 1}).then(res => {
              // 并不重新请求数据，他的意思就是说不用重新去数据库里面查询购物车，直接添加到本地的购物车就可以，本地购物车存储在localstorage中
              // 好吧好吧好吧，这里我说错了，看了才晓得，这里是两个地方都存储了，先放在了vuex里面，又添加到了localstorage里面，至于为什么，我在继续往下看
              this.ADD_CART({productId: id, salePrice: price, productName: name, productImg: img})
            })
          } else { // 未登录 vuex，，根据我的发现，这里还是存放在了localstorage中间，哪里是vuex，不不不不，是两个地方都存放了，我上面已经说了
            this.ADD_CART({productId: id, salePrice: price, productName: name, productImg: img})
          }
          // 加入购物车动画
