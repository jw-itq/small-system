...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      play () {
        // 每2.5s自动切换，setInterval是间歇调用的意思，表示每隔多长时间重复执行代码
      ...
      linkTo (item) {
        if (item.type === 0 || item.type === 2) {
          // 关联商品,&router是路由操作对象，只写对象；&route是路由信息对象，只读对象，
          // 在这里存储了之后，下次使用的时候就只需要this.&route.query.productId，这样就直接路由跳转到了path指定的路径，主要是还是做路由操作
          this.$router.push({
            path: '/goodsDetails',
            query: {
  ...
  .page {
    ...
    .dots {
      ...
      .dot-active {
        ...
        width: 15px;
        height: 15px;
        background-color: rgba(249, 248, 234, 0.82);
        border-radius: 8px;
        margin-right: 10px;
        cursor: pointer;
        border-style:solid;
