...
export default {
  ...
  [ADD_CART] (state, {productId, salePrice, productName, productImg, productNum = 1}) {
    ...
    state.cartList = cart
    // 存入localStorage,,,,,,感谢这句话让我知道了之前查询的理论是正确的
    // 那就是vuex是存放在内存中，一旦刷新就没有了
    // localStorage是存放在本地，永久不会丢失，就相当于存放在文件中吧，需要手动删除
    // sessionStorage，这个session就是回话的意思，也就是浏览器关闭了，那就没有了，也就是一次回话结束
    // cookie,这个跟前面几个都不一样的就是这个是始终会在http请求中携带的参数，始终在服务器和客户端之间传递，
    // 它要是不设置过期时间，浏览器一关闭就没有了，要是设置过期时间，就可以保存在本地磁盘中
